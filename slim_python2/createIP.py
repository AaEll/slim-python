...
def create_slim_IP(input, print_flag = False):
    ...
    if print_flag:
        def print_handle(msg):
            """Prints the time and msg (str)"""
            print_log(msg)
    else:
        def print_handle(msg):
            """ does nothing because print_flag is FALSE"""
            pass

    # check existance of fields in input (dict)
    ...
    # check dimension of fields in input (dict)
    ...
    N_pos = len(pos_ind) # number of positive examples
    N_neg = len(neg_ind) # number of negative examples
    ...
    # check number of coeffs is correct
    ...
    if not np.isnan(input['C_1']):
        ...
        C_1 = 0.5 * min(w_pos/N, w_neg/N,
                        min(C_0[L1_reg_ind] / np.sum(rho_max)))
