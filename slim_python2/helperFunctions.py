...
def print_log(msg, print_flag = True):
    ...
    Returns:
        prints time and message if print_flag is TRUE
    """
    if print_flag:
        print('{} | {}'.format(time.strftime("%m/%d/%y @ %I:%M %p", time.localtime()), msg))
        sys.stdout.flush()
...
def check_data(X, X_names, Y):
    ...
    args:
        ...
        Y (1D np.ndarray)
    Checks:
        - X,Y are np.array's and X_names is a list
        - matrix dimensions match appropriately
        - matrices are non-empty
        - warns if output is all positive or all negative
    Returns: nothing if all checks passed
    """
...
def print_slim_model(rho, X_names, Y_name, show_omitted_variables = True):
    ...
    if '(Intercept)' in rho_names:
        ...
        # remove the intercept name and value from lists
    ...
    max_name_col_length = 2 + max(len(predict_string),
                            len(total_string),
                            max([len(s) for s in rho_names]))

    max_value_col_length = 2 + max(7,
...
def get_rho_summary(rho, slim_info, X, Y):
    ...
    if np.count_nonzero(rho) > 1:
        flag = True
    else:
        flag = False

    printed_model = print_slim_model(rho,
                                    X_names = slim_info['X_names'],
                                    Y_name = slim_info['Y_name'], show_omitted_variables = flag)
...
def get_slim_summary(slim_IP, slim_info, X, Y):
    ...
    print(slim_IP.solution.get_status_string(slim_IP.solution.get_status()))
